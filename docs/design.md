...
- Path searches run on a dedicated daemon thread. The client thread incrementally captures a bounded, loaded-chunk-only terrain snapshot in fixed column batches; only the completed immutable snapshot is submitted to the search worker.
